...
const SearchResults = () => {
    ...
    // Mock product database (same as CategoryPage for consistency in demo)
    ...
    // Search logic: simple filter by title or tags
    const products = getAllProducts().filter(p => {
        const q = query.toLowerCase();
        const matchesTitle = p.title.toLowerCase().includes(q);
        const matchesTags = p.tags && p.tags.some(tag => tag.toLowerCase().includes(q));
            ...
            <section className="py-12 px-4 md:px-12 max-w-[1600px] mx-auto">
                {products.length === 0 ? (
                    <div className="text-center py-20 text-gray-500">
                        ...
                        <p>Try searching for different keywords like "shirts", "pants", or "hoodies".</p>
                    </div>
                ) : (
